...
export const CAPABILITIES = Object.freeze({
    ...
    allowedModes: MODES, scheduledOnly: true, throughServices: false
});
...
export function encodeCursor(request, version, offset = 0) {
    return Buffer.from(JSON.stringify({ policy: POLICY_VERSION, version, request, offset })).toString('base64url');
}
...
export function decodeCursor(cursor) {
    try {
        ...
        if (value.policy !== POLICY_VERSION || !/^[a-f0-9]{64}$/.test(value.version)) {
        ...
        return { version: value.version, request: normalizeRequest(value.request), offset };
    } catch (error) {
...
export function journeyID(journey, version) {
    return `${version}.${createHash('sha256').update(JSON.stringify(journey)).digest('hex').slice(0, 32)}`;
}

...
function transfer(leg, index, before, after, request, now) {
    const value = resolveConnection(index, { from: leg.from.crs, to: leg.to.crs,
    ...
    if (!value || value.mode !== leg.mode) return null;
    return { ...leg, departure: iso(value.start), arrival: iso(value.end), durationMinutes: value.minutes,
...
function warningsFor(journey, connections, request, now) {
    ...
    for (const [index, leg] of journey.legs.entries()) {
        ...
        if (Number.isFinite(arrival) && Number.isFinite(departure)
            && !transfer(leg, connections, { ...before, arrival: iso(arrival) }, { ...after, departure: iso(departure) }, request, now)) {
...
function refreshCandidate(original, annotated, network, services, connections, request, now, check, tracking) {
    const legs = [];
    ...
    for (let index = 0; index < legs.length; index++) {
        if (legs[index].kind === 'vehicle') continue;
        const updated = transfer(legs[index], connections, legs[index - 1], legs[index + 1], request, now);
        if (!updated) return { reason: 'connection' };
        legs[index] = updated;
    }
    const journey = { ...original, legs, departure: legs[0]?.departure, arrival: legs.at(-1)?.arrival,
        durationMinutes: (timeOf(legs.at(-1)?.arrival) - timeOf(legs[0]?.departure)) / MINUTE };
    if (timeOf(journey.departure) < now) return { reason: 'departed' };
    return validateJourney(journey, network, request) ? { journey } : { reason: 'connection' };
}
...
export function selectRouteBoardJourneys(journeys, limit = 5) {
    ...
    for (const journey of journeys) {
        ...
        if (!uniqueJourneys.has(key)) uniqueJourneys.set(key, journey);
    }
    ...
    return [...uniqueJourneys.values()].sort((a, b) => {
        return score(a) - score(b) || timeOf(a.arrival) - timeOf(b.arrival)
...
export async function refreshRouteBoard({ profile, network, time, limit = 5, realtime = 'apply',
    ...
    for (const candidate of candidates) {
        ...
        const value = refreshCandidate(candidate, shown, applied, serviceIndex, connections, request, current, check, tracking);
        if (value.journey) {
            const warnings = warningsFor(value.journey, connections, request, current);
    ...
    const page = presentLivePage(selectRouteBoardJourneys(journeys, limit), live, { now: current, visited, errors,
        diagnostics: snapshot.diagnostics ?? [], limited, pendingServiceIds: [...pending] });
    const needsReplan = realtime === 'apply' && (changes.length > 0 || invalidCandidates.length > 0);
    return { ...page, disruptedJourneys: selectRouteBoardJourneys(disruptedJourneys, limit), warnings: [],

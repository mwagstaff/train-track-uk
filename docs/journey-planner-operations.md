# Journey planner: production service and operations
...
## Current deployment: Sky gateway, Mini execution

### Mini production service (22 September 2026)

The production planner runs the standalone HTTP service and both routing workers on the Mini, with Mini-local Mongo for planner search history and route caches. In-flight jobs remain in process memory. The app-facing API still runs on `sky`, but its only configured planner target is **`mini`**; devices retain the same API URL. Sky no longer constructs the embedded planner or runs timetable ingestion. The Mini process binds to loopback, and an authenticated Tailscale Funnel route maps `/train-track-planner` to port 3014. The service currently uses a user LaunchAgent; unattended boot recovery remains a separate operational requirement. Sky's old timetable files were retained for recovery, not used for live planning.
...
The planner's authenticated readiness is now `ready: true`, and app-facing RAPTOR searches, queued polling, journey detail, and v4 live route boards have passed through Sky's gateway. TubeTrack is enabled. Search history remains in Mini-local Mongo, not Sky's Mongo. The process and Funnel restoration are still login-scoped, so a reboot/FileVault unlock may require operator attention. Stopping Mini now makes app-facing planning unavailable; drain or announce an interruption before stopping it. Stop it with:

```sh
/Users/mwagstaff/dev/server-tooling/deploy/stop_node_project.zsh train-track-planner mini
```

Mini Mongo can be provisioned with `deploy/train_track_planner_mongo.zsh check` and `deploy/train_track_planner_mongo.zsh setup` from server-tooling. Setup refuses existing users/credentials, creates a dedicated `train_track_planner` database account and a local admin recovery credential, enables authorization, restarts Mongo, verifies unauthenticated reads are denied, and adds the connection to the existing planner credential file. Both Mongo and the planner HTTP service stay bound to loopback. Back up `/Users/mwagstaff/.local/share/train-track-planner/mongodb-admin.password` securely; database files need a separate regular backup and restore test.

The deployed application, live timetable storage, logs, monitoring job, and deployment helpers now use the production naming. The deployment remains a user LaunchAgent; this naming migration does not change its login dependency. Moving it to a system LaunchDaemon still requires administrator authorization and verification of Mongo, Funnel, FileVault recovery, backup/restore, and log rotation. There is one planner deployment definition, so a second configuration cannot accidentally compete for port 3014.

The 22 September naming migration preserved the live dataset version and rewrote its active/rollback and ingestion metadata paths to the new data directory. The unused staged checkout/data and pre-migration configuration are retained privately under `/Users/mwagstaff/.local/share/train-track-planner/migration-backups/20260922T181759Z`; these are historical recovery records, not active configuration. The service was restarted with no pending planner work or ingestion, then passed loopback smoke checks and public gateway queued-search/detail checks. New host-labelled timing rows were verified in Mini Mongo. Ingestion reported `unchanged` without an error, and Sky's renamed monitoring target was up with five rules loaded. The renamed alert-rule test passed. All 14 focused planner tests passed; the wider deployment-tooling suite passed 12 of 13 tests, with an unchanged test expecting a monitoring message that the current deployer no longer prints for a non-monitoring host.

### Separate planner execution on the Mac Mini

The app-facing API remains on `sky`. It contains a target registry and gateway for the complete v3/v4 planner namespace; devices keep their existing API URL. The registry contains only Mini and the embedded Sky planner is disabled. A separately authenticated process runs the planner routes on the Mini with:
...
PLANNER_DEFAULT_TARGET=mini
PLANNER_LEGACY_TARGET=mini
PLANNER_EMBEDDED_ENABLED=false
PLANNER_INGESTION_ENABLED=false
PLANNER_HOST_ID=sky
```

`PLANNER_FORCE_TARGET` is an emergency override and locks target changes, but can only name a configured target. The active target otherwise lives in the `planner_configuration` Mongo collection with a revision check. The public Journey Planner admin page displays the selected target but cannot change it. The Sky-only operator command below checks candidate identity, protocol and readiness. Jobs, details, cursors, and idempotent retries use expiring records in `planner_routing_ownership`. A cutover audit found no unexpired Sky-owned artifacts before removing Sky from the target registry.

The command is intentionally not exposed over HTTP. From an operator terminal, inspect the persisted target with:

```sh
ssh sky 'cd /home/mwagstaff/dev/train-track-api && set -a && . ./.static-config-train-track-api.env.sh && . ./.bw-secrets.env.sh && set +a && /home/mwagstaff/.local/share/train-track-api/runtime/node24/bin/node scripts/planner-target-admin.js status'
```

The persisted selection was last verified as Mini at revision 4. The command cannot select Sky now that Sky is absent from the registry. A rollback to embedded execution requires restoring the saved Sky registry at `/home/mwagstaff/.local/share/train-track-api/planner-targets.json.before-mini-only-20260921`, changing the Sky deployment configuration to enable the embedded service and current ingestion, redeploying, and checking readiness before selection. Do not simply select Sky or assume its retained timetable is current.
...
The canonical `train-track-journey-planner` entry in server-tooling (aliases `journey-planner` and `train-track-planner`) deploys `planner-server.js` to `/Users/mwagstaff/dev/train-track-planner`, pins a dedicated Node 24 runtime, persists data at `/Users/mwagstaff/.local/share/train-track-planner/planner`, and writes output below `/Users/mwagstaff/Library/Logs/train-track-planner`. Its user LaunchAgent is `com.train-track-planner.api`. The migration preserves the active and rollback datasets, credentials, Mongo database, host identity, port, and public Funnel URL.

```sh
/Users/mwagstaff/dev/server-tooling/deploy/node_project.zsh journey-planner mini --full --no-tail
```

A normal deployment preserves the working `.bw-secrets.planner.env.sh` unless Bitwarden synchronization is explicitly requested. Before a Bitwarden sync, ensure the required planner Mongo URI, service token, S3 keys, and live provider keys exist under the `train-track-journey-planner` application tag. The service token must continue matching the gateway token on `sky`.

A Mini audit on 21 September 2026 found that Funnel is still restored by the valid login-scoped `com.mike.tailscale-funnel-apply` LaunchAgent. The similarly named file under `/Library/LaunchDaemons` contained shell installation commands rather than a valid plist and was not loaded. The managed Funnel apply script is `/Users/mwagstaff/dev/server-tooling/deploy/tailscale-funnel-apply.sh`; it was installed at `/Users/mwagstaff/bin/tailscale-funnel-apply.sh` with a dated backup of the previous script. Replace the invalid system file with a reviewed service before relying on Funnel after reboot/FileVault unlock.

For release acceptance, validate authenticated health/readiness, cold and warm searches, queued polling/cancellation, detail, v3/v4 boards, disruption profiles, ingestion, metrics, and central search logs from `sky` through the Funnel URL. Earlier deployment checks covered the main search/queued/detail/v4-board paths, not this whole release checklist. Recovery to Sky now requires a configuration and service rollback, not a one-command target change. The admin page itself remains publicly readable and its cache-clear control is not operator-authenticated; add a proper admin access boundary before exposing further controls or treating the setup as generally available.
...
- `static_env.PLANNER_EMBEDDED_ENABLED=false` and `PLANNER_INGESTION_ENABLED=false`: no Sky-local planner execution or timetable checks.
...
Full deployments use the same runtime pin and exclusions. A missing pinned runtime fails before deployment changes begin. Other projects retain their existing runtime selection. Timetable ingestion is role-explicit: it is enabled on Mini and disabled on Sky. Credentials alone no longer activate the API host's scheduler. Existing excluded source-tree copies are protected by rsync; they are not the production data source.

### Historical Sky-local timetable procedure (inactive)

The supplied RJTTF939 snapshot was validated on Sky and moved to `PLANNER_DATA_DIR/snapshots/3d9d573635ed619ac3808338176858077f4d35650846ba0a0e829ed53ad64f5c`, then activated with the CLI. This data is retained but is **not** used by the current Mini-only target. The Sky API unit is `com.train-track-api.api.service`; its wrapper and static configuration are regenerated by the deployer.

The following commands document the former Sky-local procedure. New monthly feeds are now handled by Mini's managed ingestion; do not use these commands for live planner data. Replace the source and candidate paths below only for an intentional Sky recovery:
